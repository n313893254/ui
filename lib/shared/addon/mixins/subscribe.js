...
export default Mixin.create({
    ...
    socket.on('message', (event) => {
      ...
      schedule('actions', this, function() {
        ...
        case 'resource.remove':
          queue.enqueue(d);
          if (d.data.type === 'deployment' && d.name === 'resource.remove') {
  ...
  processQueue() {
    ...
    while ( event ) {
      if (event.data.type === 'deployment' && event.name === 'resource.remove') {
        debugger
      }
      if ( !event.data ) {
        continue;
      ...
      if (forceRemove && event.data.type === 'deployment') {
        debugger
      }
      ...
      if ( get(this, 'updateProjectStore') && projectId && projectStore.hasType(type) ) {
        ...
        if (type === 'deployment') {
          projectStore.findAll('workload').then(res => console.log(res.content, 'updateStore'))
        }
      }
  ...
  subscribeConnected(tries,msec) {
    set(this,'queueTimer', setInterval(this.processQueue.bind(this), 5000));
