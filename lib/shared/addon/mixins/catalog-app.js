...
export default Mixin.create({
  ...
  answers: computed('selectedTemplateModel.questions.@each.{variable,answer}', function() {
    const out = {};
    console.log(get(this, 'selectedTemplateModel.questions'), 'selectedTemplateModel.questions');
    (get(this, 'selectedTemplateModel.questions') || []).forEach((item) => {
      if (item.type === 'object') {
        (item.fields || []).map(f => {
          out[`${item.variable}.${f.variable}`] = stringifyAnswer(f.answer);
          (get(f, 'subquestions') || []).forEach((sub) => {
      ...
      if (item.type === 'array') {
        (item.fields || []).map((f, index) => {
          out[`${item.variable}[${index}]`] = stringifyAnswer(f.answer);
          (get(f, 'subquestions') || []).forEach((sub) => {
            out[sub.variable] = stringifyAnswer(sub.answer);
          });
        })
