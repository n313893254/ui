...
export function satisfies(version, range) {
  ...
  const out =Semver.satisfies(Semver.coerce(version), range)
  console.log(out, 'satisfies-out')
  return out;
}
...
export function maxSatisfying(versions, range) {
  ...
  nueVersions.forEach( ( version, i, ary ) => {
    ...
    if ( !Semver.valid(Semver.coerce(version)) ) {
      ...
      coercedVersions.push(Semver.coerce(version).version)
      originVersions.push(version)
    }
  });
  ...
  if (satisfiedVersion) {
    return originVersions.find((v) => {
      const value = Semver.coerce(v).version
      return value.includes(satisfiedVersion);
    });
  } else {
    return null;
  }
