...
export default Component.extend(ClusterDriver, {
  ...
  init() {
    ...
    if ( !config ) {
      config = this.get('globalStore').createRecord({
        ...
        location:          'eastus',
        addonEnableHTTPApplicationRouting: false,
        addonEnableMonitoring: true,
      });
  ...
  willSave() {
    const addonEnableMonitoring = get(this, 'config.addonEnableMonitoring')
    const config = get(this, 'config')

    if (addonEnableMonitoring) {
      setProperties(config, {
    ...
    const networkPolicy = get(this, 'networkPolicy')

    if (networkPolicy) {
      set(config, 'networkPolicy', 'Calico')
    } else {
      set(config, 'networkPolicy', null)
    }
