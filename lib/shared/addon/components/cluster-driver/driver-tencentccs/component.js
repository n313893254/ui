...
const ROOT_DISK = [
  {
    label: 'clusterNew.tencentccs.disk.basic',
    value: 'LOCAL_BASIC'
  },
  {
    label: 'clusterNew.tencentccs.disk.ssd',
    value: 'LOCAL_SSD'
  },
  {
    label: 'clusterNew.tencentccs.disk.cloudBasic',
    value: 'CLOUD_BASIC'
  },
  {
    label: 'clusterNew.tencentccs.disk.cloudSsd',
    value: 'CLOUD_SSD'
  }
];
...
const STORAGE_DISK = [
  ...
  {
    label: 'clusterNew.tencentccs.disk.cloudPremium',
    value: 'CLOUD_PREMIUM'
  },
  {
    label: 'clusterNew.tencentccs.disk.cloudSsd',
...
const BAND_WIDTH = [
  {
    label: 'clusterNew.tencentccs.bandwidthType.hour',
    value: 'PayByHour'
  },
  {
    label: 'clusterNew.tencentccs.bandwidthType.traffic',
    value: 'PayByTraffic'
  }
];
...
export default Component.extend(ClusterDriver, {
  intl:        service(),
  layout,
  configField: 'tencentContainerCloudServiceConfig',
  ...
  init() {
    ...
    let config = get(this, 'cluster.tencentContainerCloudServiceConfig');

    if ( !config ) {
      config = this.get('globalStore').createRecord({
        type:           'tencentContainerCloudServiceConfig',
      ...
      set(this, 'cluster.tencentContainerCloudServiceConfig', config);
    }
  },
  ...
  actions: {
    tencentLogin(cb) {
      ...
      if ( !secretId ) {
        errors.push(intl.t('clusterNew.tencentccs.secretId.required'));
      }

      if ( !secretKey ) {
        errors.push(intl.t('clusterNew.tencentccs.secretKey.required'));
      }
    ...
    loadNodeConfig(cb) {
      ...
      const clusterCidr = get(this, 'config.clusterCidr');
      const vpcId = get(this, 'config.vpcId');
      const goodsNum = get(this, 'config.goodsNum');

      if ( !clusterCidr ) {
        errors.push(intl.t('clusterNew.tencentccs.cidr.required'));
      }

      if ( !vpcId ) {
        errors.push(intl.t('clusterNew.tencentccs.vpc.required'));
      }

      if ( !goodsNum ) {
        errors.push(intl.t('clusterNew.tencentccs.goodsNum.required'));
      }
    ...
    loadInstanceConfig(cb) {
      ...
      if ( !zoneId ) {
        errors.push(intl.t('clusterNew.tencentccs.zone.required'));
      }

      if ( !subnetId ) {
        errors.push(intl.t('clusterNew.tencentccs.subnet.required'));
      }
    ...
    save(cb) {
      ...
      const sgId = get(this, 'config.sgId');
      const keyId = get(this, 'config.keyId');

      if ( !sgId ) {
        errors.push(intl.t('clusterNew.tencentccs.securityGroup.required'));
      }

      if ( !keyId ) {
        errors.push(intl.t('clusterNew.tencentccs.keyPair.required'));
      }
