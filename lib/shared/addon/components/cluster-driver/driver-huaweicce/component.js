...
export default Component.extend(ClusterDriver, {
  ...
  init() {
    ...
    if ( !config ) {
      config = this.get('globalStore').createRecord({
        ...
        billingMode: 0,
        containerNetworkMode: 'underlay_ipvlan',
      ...
      if (clusterType === 'VirtualMachine') {
        set(this, 'vmNodeFlavor', get(this, 'config.nodeFlavor'))
      } else {
        set(this, 'bmNodeFlavor', get(this, 'config.nodeFlavor'))
      }
    ...
    k8sStore.rawRequest({
      url:    `${k8sStore.baseUrl}/businesses?action=getHuaweiCloudRegion`,
      method: 'POST',
    }).then(res => {
      const {body={}} = res
  ...
  nodeOperationSystemContent: computed('config.clusterType', function() {
    const clusterType = get(this, 'config.clusterType')
    if (clusterType === 'VirtualMachine') {
      return [{
        label: 'EulerOS 2.2',
  ...
  clusterTypeChange: observer('config.clusterType', function() {
    ...
    set(this, 'config.highwaySubnet', filter[0] && filter[0].subnets[0])
    set(this, 'highAvailabilityEnabled', 't2')
    set(this, 'config.nodeOperationSystem', 'EulerOS 2.3')
    set(this, 'config.billingMode', 2)
    set(this, 'config.containerNetworkMode', 'underlay_ipvlan')
  }),
  ...
  nodeFlavorContent: computed('nodeFlavors.[]', 'config.clusterType', 'bareMetalFlavors.[]', function() {
    const nodeFlavors = get(this, 'nodeFlavors') || []
    const bareMetalFlavors = get(this, 'bareMetalFlavors') || []
    const clusterType = get(this, 'config.clusterType')
    if (clusterType === 'VirtualMachine') {
      return nodeFlavors.map(n => {return {label: `${n.name} ( vCPUs: ${n.vcpus}, memory: ${n.ram / 1024} GB )`, value: n.name}})
    }
    if (clusterType === 'BareMetal') {
      return bareMetalFlavors.map(n => {return {label: `${n.name} ( vCPUs: ${n.vcpus}, memory: ${Math.round(n.ram / 1024)} GB )`, value: n.name}})
    }
  }),
  ...
  networkContent: computed('networks.[]', function() {
    ...
    return arr.map(a => ({label: a.name, value: a.subnets && a.subnets[0]}))
  }),
  ...
  loginModeChange: observer('loginMode', function() {
    ...
    if (loginMode === 'keyPair') {
      ...
      set(this, 'config.username', null)
    }
    if (loginMode === 'password') {
      set(this, 'config.sshKey', null)
      set(this, 'config.username', 'root')
    }
  }),

  passwordChange: observer('preBase64Password', function() {
    const preBase64Password = get(this, 'preBase64Password') || ''
    set(this, 'passwordError', null)
    const passwordError = get(this,'passwordError')||[];
    const lengthRegex = /^.*(?=.{8,26})/
    if (!lengthRegex.test(preBase64Password)) {
      passwordError.pushObject('Must be 8 to 26 characters long.')
    }
    const characterReverseRegex = /[^!@$%\^\-_=+&\*?\(\)\{\}:,.\/\w]/
    if (characterReverseRegex.test(preBase64Password)) {
      passwordError.pushObject('Must be digits, uppercase letter, lowercase letter or [!@$%^_-+=&*?(){}:,./]')
    }
    let count = 0
    const uppercaseLetterRegex = /^.*(?=.*[A-Z])/
    const lowercaseLetterRegex = /^.*(?=.*[a-a])/
    const specialCharactersRegex = /^.*(?=.*[!@$%\^\-_=+&\*?\(\)\{\}:,.\/])/
    const digitRegex = /^.*(?=.*\d)/
    if (uppercaseLetterRegex.test(preBase64Password)) {
      count += 1
    }
    if (lowercaseLetterRegex.test(preBase64Password)) {
      count += 1
    }
    if (specialCharactersRegex.test(preBase64Password)) {
      count += 1
    }
    if (digitRegex.test(preBase64Password)) {
      count += 1
    }
    if (count < 3) {
      passwordError.pushObject('Must contain at least three types character in digits, uppercase letter, lowercase letter and spacial character')
    }
    set(this, 'passwordError', passwordError);
  }),
  ...
  willSave() {
    ...
    if (get(this, 'mode') === 'new') {
      ...
      if (get(this, 'loginMode') === 'password') {
        const preBase64Password = get(this, 'preBase64Password') || ''
        const hikari = sha512crypt(preBase64Password, 'V6azyeLwcD3CHlpY')
        set(this, 'config.password', AWS.util.base64.encode(hikari))
      }
    }

    const clusterType = get(this, 'config.clusterType')
    if (clusterType === 'VirtualMachine') {
      set(this, 'config.nodeFlavor', get(this, 'vmNodeFlavor'))
    } else {
      set(this, 'config.nodeFlavor', get(this, 'bmNodeFlavor'))
    }
  ...
  checkout(cb) {
    ...
    filter.doAction('checkout', {
      ...
      url: `${k8sStore.baseUrl}/business/${businessId}?action=checkout`
    }).then(() => {
      cb()
    })
  },
  ...
  validate() {
    ...
    if (get(this, 'loginMode') === 'keyPair') {
      return errors.length === 0
    } else {
      return (errors.length === 0) && (passwordError.length === 0)
    }
  },
  ...
  actions: {
    checkAccount(cb) {
      ...
      if (get(this, 'authConfigred')) {
        ...
        filter.doAction('getHuaweiCloudApiInfo', {
          ...
          url: `${k8sStore.baseUrl}/business/${businessId}?action=getHuaweiCloudApiInfo`
        }).then(res => {
          console.log(res, 'res')

          const bareMetalFlavors = res.bareMetalFlavors.filter(b => (b.name || '').startsWith('physical')) || []
          ...
          if (get(this, 'mode') === 'new') {
            setProperties(this, {
              ...
              'vmNodeFlavor': res.flavors[0].name,
              'bmNodeFlavor': bareMetalFlavors[0].name,
    ...
    configreNode(cb) {
      ...
      try {
        const location = window.location;
        const region = get(this, 'config.zone')
        let endpoint = `bms.${region}.myhuaweicloud.com`;
        endpoint = get(this,'app.proxyEndpoint') + '/' + endpoint.replace('//', '/');
        endpoint = `${location.origin}${endpoint}`;

        let client = new HW.ECS({
          ak: get(this, 'config.accessKey'),
          sk: get(this, 'config.secretKey'),
          projectId: get(this, 'config.projectId'),
          endpoint,
          region: region,
          toSignedHost: `bms.${region}.myhuaweicloud.com`,
          service: 'bms',
        })

        client.getBareMetalFlavor((err, response) => {
          if (err) {
            let errors = this.get('errors')||[];
            errors.pushObject(err);
            set(this, 'errors', errors);
          ...
          const bareMetalFlavors = response.body.flavors.filter(b => (b.name || '').startsWith('physical')) || []
          set(this, 'bareMetalFlavors', bareMetalFlavors)

          if (get(this, 'mode') === 'new') {
            set(this, 'bmNodeFlavor', bareMetalFlavors[0] && bareMetalFlavors[0].name || null)
          }
        })
      } catch (err) {
        const errors = get(this, 'errors') || [];
