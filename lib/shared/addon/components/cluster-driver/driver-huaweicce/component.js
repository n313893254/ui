...
export default Component.extend(ClusterDriver, {
  ...
  authConfigred: computed('config.businessId', 'businessName', function() {
    const businessId = get(this, 'config.businessId')
    const businessName = get(this, 'businessName')
    if ((businessId && businessId !== 'null') || (businessName && businessName !== 'No business')) {
      return true
    }
    return false
  }),
  ...
  init() {
    ...
    k8sStore.rawRequest({
      ...
      set(this, 'masterVersions', masterVersions.map(m => ({lable: m, value: m})))
    ...
    if ( !config ) {
      ...
      const businessId = business[0] && business[0].id
      if (!businessId) {
  ...
  clusterTypeChange: observer('config.clusterType', function() {
    ...
    // const filter = networks.filter(n => n.status === 'ACTIVE' && n.tenant_id === get(this, 'config.projectId') && n[`provider:network_type`] === 'geneve')
    const filter = networks.filter(n => n.status === 'ACTIVE' && n.tenant_id === get(this, 'config.projectId'))
  ...
  networkContent: computed('networks.[]', function() {
    const networks = get(this, 'networks')
    let arr = networks.filter(n => n.status === 'ACTIVE' && n.tenant_id === get(this, 'config.projectId') && n[`provider:network_type`] === 'geneve')
  ...
  businessName: computed('model.business.[]', function() {
    ...
    const businessId = labels.business
    if (!businessId) {
      return 'No business'
    }
  ...
  willSave() {
    ...
    set(labels, 'business', businessId)
  ...
  actions: {
    checkAccount(cb) {
      ...
      if (get(this, 'authConfigred')) {
        const labels = get(this, 'config.labels') || {}
        const k8sStore = get(this, 'k8sStore')
        const businessId = get(this, 'config.businessId') || labels.business
        const business = get(this, 'model.business') && get(this, 'model.business').content || []
        const filter = business.filter(b => b.id === businessId)[0]
