...
export default Component.extend(ClusterDriver, {
  intl:         service(),
  layout,
  configField:  'aliyunContainerServiceForKubernetesConfig',
  aliyunClient: null,
  ...
  init() {
    ...
    let config = get(this, 'cluster.aliyunContainerServiceForKubernetesConfig');

    if ( !config ) {
      config = this.get('globalStore').createRecord({
        type:                     'aliyunContainerServiceForKubernetesConfig',
        accessKeyId:              null,
      ...
      set(this, 'cluster.aliyunContainerServiceForKubernetesConfig', config);
    }
  },
  ...
  fetch(resource, plural, page = 1) {
    ...
    if ( !ecs ) {
      ecs = new ALY.ECS({
        ...
        endpoint:        `/meta/proxy/https:/${ ENDPOINT }`,
      });
    }
