        ...
        if ( resource.cb ) {
          ...
          if (resource.type === 'business') {
            resource.delete({url:`${k8sStore.baseUrl}/v3/business/${resource.id}`}).then(() => {
              k8sStore._remove('business', resource)
            }).finally(cb)
          } else if (resource.type === 'businessRoleTemplate') {
            resource.delete({url:`${k8sStore.baseUrl}/v3/businessRoleTemplate/${resource.id}`}).then(() => {
              console.log(k8sStore, 'ssdfsdfsfd')
              k8sStore._remove('businessRoleTemplate', resource)
            }).finally(cb)
          } else {
