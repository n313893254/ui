...
export default Service.extend({
  ...
  init() {
    ...
    hash({
      allClusters: globalStore.all('cluster'),
      allProjects: globalStore.all('project'),
      business: k8sStore.findAll('business', {url:`${k8sStore.baseUrl}/v3/business`, forceReload: true}).catch(err => console.log(err)),
    }).then(hash => {
      const {allClusters=[], business=[]} = hash
      const username = get(this, 'access.me.username')
      if (username !== 'admin') {
        const businessIds = business.map(b => b.id)
        const filter = allClusters.filter(c => {
        ...
        setProperties(this, {
          allClusters: filter,
          allProjects: hash.allProjects,
        });
      } else {
        setProperties(this, {
          allClusters: hash.allClusters,
          allProjects: hash.allProjects,
        });
      }
    })
