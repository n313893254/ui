...
import {fromByteArray} from 'shared/utils/base64-js';
...
export default Component.extend(NewOrEdit, {
  ...
  actions: {
    save(cb) {
      ...
      if (targetType === 'kafka' && get(model, 'kafkaConfig.zookeeperEndpoint')) {
        setProperties(model.kafkaConfig, {
          keytab: null,
          principal: null,
        })
      } else {
        console.log(model.kafkaConfig.keytab, 'model.kafkaConfig.keytab')
        // set(model.kafkaConfig, 'keytab', AWS.util.base64.encode(model.kafkaConfig.keytab))
        console.log(get(model, 'kafkaConfig'), 'kafkaConfig')
      }
      this._super(cb);
    },
  },
