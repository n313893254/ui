...
export default Component.extend({
  ...
  validateNotification() {
    ...
    if (condition.length > 0 && recipients.length > 0) {
      const {notifier, recipient} = recipients[0]
      if (!notifier || !recipient) {
        errors.pushObject(intl.t('pipelineNotification.recipients.required'))
      }
    }
    if (condition.length === 0) {
      const {notifier, recipient} = recipients[0]
      if (notifier || recipient) {
        errors.pushObject(intl.t('pipelineNotification.condition.required'))
      } else {
        errors.pushObject(intl.t('pipelineNotification.condition.required'))
        errors.pushObject(intl.t('pipelineNotification.recipients.required'))
      }
